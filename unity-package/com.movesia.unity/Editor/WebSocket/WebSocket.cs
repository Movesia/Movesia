...
namespace NativeWebSocket
{
    ...
    public enum WebSocketCloseCode
    {
        ...
        // Custom codes (4000-4999) — match server-side CloseCode in types.ts
        Superseded = 4001,
        DuplicateSession = 4002,
        AuthenticationFailed = 4003,
        SessionExpired = 4004,
        CompilationReset = 4005,
        ProjectMismatch = 4006,
    }
    ...
    public class WebSocket : IWebSocket
    {
        ...
        public async Task Connect()
        {
            ...
            catch (Exception ex)
            {
                ...
                // Try to read the actual close code from the underlying socket
                var code = WebSocketCloseCode.Abnormal;
                if (m_Socket.CloseStatus.HasValue)
                {
                    code = WebSocketHelpers.ParseCloseCodeEnum((int)m_Socket.CloseStatus.Value);
                }
                OnClose?.Invoke(code);
            }
            finally
            ...
            try
            {
                while (m_Socket.State == System.Net.WebSockets.WebSocketState.Open)
                {
                    ...
                    using (var ms = new MemoryStream())
                    {
                        ...
                        {
                            int rawCode = result.CloseStatus.HasValue
                                ? (int)result.CloseStatus.Value
                                : (int)WebSocketCloseCode.NoStatus;
                            await Close();
                            closeCode = WebSocketHelpers.ParseCloseCodeEnum(rawCode);
                            break;
                        }
                    }
                }
