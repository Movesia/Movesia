...
public static class MaterialManager
{
    ...
    private static (Texture texture, string error) ResolveTexture(JToken value)
    {
        ...
        if (value.Type == JTokenType.Integer)
        {
            ...
            if (instanceId == 0)
                return (null, null);
            var obj = EditorCompat.IdToObject(instanceId) as Texture;
            if (obj == null)
        ...
        int assignSlotIndex = 0)
    {
        try
        {
            ...
            if (instanceId != 0)
            {
                material = EditorCompat.IdToObject(instanceId) as Material;
            }
            ...
            {
                var go = EditorCompat.IdToObject(assignToGameObject) as GameObject;
                if (go == null)
                {
                    return new MaterialResult
                    {
